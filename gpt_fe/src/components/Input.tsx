  ...
  return (
    <div className="items-left z-5 flex h-fit w-full flex-col rounded-xl text-lg text-slate-12 md:flex-row md:items-center">
      ...
      {isTypeTextArea() ? (
        <textarea
          className={clsx(
            "delay-50 h-15 input-bg w-full resize-none  rounded-xl p-2 text-sm tracking-wider text-slate-8 outline-none transition-all  placeholder:text-slate-8 focus:border-none focus:outline-none sm:h-20 md:text-lg",
        ...
        <input
          className={clsx(
            "border-1 w-full rounded-md border-shade-600-dark p-2 py-1 text-sm tracking-wider text-slate-12 outline-none transition-all duration-200  placeholder:text-slate-8 focus:border-none focus:outline-none sm:py-3 md:text-lg",
            props.disabled && "cursor-not-allowed",
            props.left && "md:rounded-l-none",
            props.small && "text-sm sm:py-[0]"
          )}
