  ...
  return (
    <div>
      {props.label && (
        <label
          htmlFor={props.name}
          className="flex items-center gap-2 text-sm font-bold leading-6 text-slate-5"
        >
          {props.icon}
          <span>{props.label}</span>

          {props.type == "range" && (
            <span className="flex items-center gap-2 text-sm font-bold leading-6 text-slate-1">
              ({props.value})
            </span>
          )}
        </label>
      ...
      <div className="relative flex flex-col gap-1 rounded-md shadow-sm">
        ...
        <div className="flex flex-grow flex-row items-center gap-1">
          <input
            ...
            id={props.name}
            className={clsx(
              "placeholder:text-color-tertiary focus:outline-inset block w-full rounded-md border-0 bg-shade-200-light  p-1.5 text-slate-5 shadow-depth-1 transition-colors sm:text-sm sm:leading-6"
            )}
